...
GW_ENV_FILE="${GW_ENV_FILE:-${GW_DIR}/env.local-demo.example}"
...
SUITE_SECTOR="${SECTOR:-animal-care}"
SUITE_EMPLOYEE_ROLE="${EMPLOYEE_ROLE:-ISCO-08|2250}"
...
(
  ...
  if [[ -n "${ICA_ENV_FILE}" ]]; then
    ...
    ICA_KNOWN_CERTS_AUTO_DOWNLOAD="${ICA_KNOWN_CERTS_AUTO_DOWNLOAD:-true}" \
    DID_WEB_DOMAIN="${DID_WEB_DOMAIN:-127.0.0.1:${ICA_PORT}}" \
...
SECTOR="${SUITE_SECTOR}" \
EMPLOYEE_ROLE="${SUITE_EMPLOYEE_ROLE}" \
